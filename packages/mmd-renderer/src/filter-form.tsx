...
import { filterMetadata, isEmptyFilter, normalizeFilters, validateFilterValue } from "./filter-values";
...
export function FilterForm({ fields, value = emptyValues, onSearch, layout = "compact" }: FilterFormProps) {
  ...
  return <Form<MmdRecord> form={form} layout={layout === "compact" ? "vertical" : "inline"}
    ...
    <div className={layout === "compact" ? "mmd-filter-grid" : "mmd-filter-inline"}>
      {available.map((field) => <Form.Item key={field.name} name={field.name}
        hidden={layout === "compact" && !expanded && filterMetadata(field).primary === false && isEmptyFilter(value[field.name])}
        label={translateMetadataLabel(t, "fields", field.name, field.label)}
        rules={[{ validator: (_rule, next: unknown) => {
          const error = validateFilterValue({ field, value: next, locale });
          return error ? Promise.reject(new Error(error)) : Promise.resolve();
        } }]}>
        <MmdField field={field} scene="search" value={undefined} record={record} />
      </Form.Item>)}
    </div>
    <Space className="mmd-search-actions" wrap>
      <Button type="primary" htmlType="submit">{t("common.search")}</Button>
      <Button onClick={() => { form.resetFields(); onSearch({}); }}>{t("common.reset")}</Button>
      {layout === "compact" && advanced.length > 0 ? <Button type="link" aria-expanded={expanded}
        onClick={() => setExpanded((value) => !value)}>
        {locale === "en-US" ? (expanded ? "Fewer filters" : `More filters (${advanced.length})`)
          : (expanded ? "收起筛选" : `更多筛选（${advanced.length}）`)}
      </Button> : null}
    </Space>
  </Form>;
}
